...
import { loadResidentProfile, resolveResidenceInBackground, writeProfileExtras } from '../lib/residentProfile';
...
import type { Society } from '../api/society';
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await blockApi.fetchBlocksBySociety(selectedSocietyId);
        if (!cancelled) setBlocks((res?.data ?? []) as Block[]);
      } catch {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await flatApi.fetchFlatsByBlock(selectedBlockId);
        if (!cancelled) setFlats((res?.data ?? []) as Flat[]);
      } catch {
  ...
  const handleSave = async () => {
    ...
    const nextSocietyId = selectedSocietyId || (user.society_id != null ? String(user.society_id) : '');
    const nextFlatId = selectedFlatId || (user.flat_id != null ? String(user.flat_id) : '');
    const currentSocietyId = user.society_id != null ? String(user.society_id) : '';
    const currentFlatId = user.flat_id != null ? String(user.flat_id) : '';
    if (nextSocietyId && nextSocietyId !== currentSocietyId) {
      updates.society_id = nextSocietyId;
    }
    if (nextFlatId && nextFlatId !== currentFlatId) {
      updates.flat_id = nextFlatId;
    }
    ...
    const profilePayload: residentApi.ResidentUpdate = {
      name: fullName,
      email: fullEmail,
      phone_number: fullPhone,
      ...(updates.society_id != null ? { society_id: updates.society_id } : {}),
      ...(updates.flat_id != null ? { flat_id: updates.flat_id } : {}),
    };
    ...
    try {
      ...
      if (avatarChanged) {
        if (avatarDraft === null) {
          updateRes = await residentApi.residentUpdate(user.id, profilePayload, { removeProfileImage: true });
        } else if (avatarDraft?.uri) {
          updateRes = await residentApi.residentUpdate(user.id, profilePayload, {
            profileImage: {
              uri: avatarDraft.uri,
      ...
      if (raw && typeof raw === 'object') {
        const nextSocietyIdValue = updates.society_id ?? (raw as any).society_id ?? user?.society_id ?? null;
        const nextFlatIdValue = updates.flat_id ?? (raw as any).flat_id ?? user?.flat_id ?? null;
        dispatch(
          setAuthUser({
            ...(user ?? {}),
            ...
            society_id: nextSocietyIdValue,
            flat_id: nextFlatIdValue,
            profile_image: (raw as any).profile_image ?? user?.profile_image ?? null,
          })
        );
        if (user?.id != null && (updates.society_id != null || updates.flat_id != null)) {
          await writeProfileExtras(user.id, {
            society_id: nextSocietyIdValue,
            flat_id: nextFlatIdValue,
          });
        }
  ...
  const startEditing = async () => {
    ...
    const flatId = user?.flat_id != null ? String(user.flat_id) : '';
    ...
    try {
      ...
      if (societyId && flatId) {
        const blocksRes = await blockApi.fetchBlocksBySociety(societyId);
        const blockList = (blocksRes?.data ?? []) as Block[];
        setBlocks(blockList);
        for (const block of blockList) {
          const flatsRes = await flatApi.fetchFlatsByBlock(block.id);
          const flatList = (flatsRes?.data ?? []) as Flat[];
          const match = flatList.find((f) => String(f.id) === flatId);
          if (match) {
    ...
    <>
      ...
      >
            ...
            <View style={styles.modalCard}>
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => !societiesLoading && setSocietyModalVisible(true)}
                  disabled={loading || societiesLoading}
                  activeOpacity={0.9}
                >
                  <Text style={styles.selectorLabel}>Society</Text>
                  <Text style={styles.selectorValue}>
                    {societiesLoading ? 'Loading...' : selectedSociety?.name || 'Select society'}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.selector, !selectedSocietyId && styles.selectorDisabled]}
                  onPress={() => selectedSocietyId && !blocksLoading && setBlockModalVisible(true)}
                  disabled={loading || !selectedSocietyId || blocksLoading}
                  activeOpacity={0.9}
                >
                  <Text style={styles.selectorLabel}>Block</Text>
                  <Text style={styles.selectorValue}>
                    {blocksLoading ? 'Loading...' : selectedBlock?.name || 'Select block'}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.selector, !selectedBlockId && styles.selectorDisabled]}
                  onPress={() => selectedBlockId && !flatsLoading && setFlatModalVisible(true)}
                  disabled={loading || !selectedBlockId || flatsLoading}
                  activeOpacity={0.9}
                >
                  <Text style={styles.selectorLabel}>Flat</Text>
                  <Text style={styles.selectorValue}>
        ...
        <Modal visible={societyModalVisible} transparent animationType="fade" onRequestClose={() => setSocietyModalVisible(false)}>
          <Pressable style={styles.modalOverlay} onPress={() => setSocietyModalVisible(false)} />
          <View style={styles.modalCenter}>
            <View style={styles.modalCard}>
              ...
              <FlatList
                ...
                ListEmptyComponent={
                  <Text style={styles.modalHint}>{societiesLoading ? 'Loading…' : 'No societies found'}</Text>
                }
              />
            </View>
          </View>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    style={styles.pickerItem}
                    onPress={() => {
                      setSelectedBlockId(String(item.id));
                      setSelectedFlatId('');
                ...
                ListEmptyComponent={
                  <Text style={styles.modalHint}>{blocksLoading ? 'Loading…' : 'No blocks found'}</Text>
                }
              />
            </View>
          </View>
        ...
        <Modal visible={flatModalVisible} transparent animationType="fade" onRequestClose={() => setFlatModalVisible(false)}>
          <Pressable style={styles.modalOverlay} onPress={() => setFlatModalVisible(false)} />
          <View style={styles.modalCenter}>
            <View style={styles.modalCard}>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    style={styles.pickerItem}
                    onPress={() => {
                      setSelectedFlatId(String(item.id));
                      setFlatModalVisible(false);
                    }}
                  >
                    <Text style={styles.pickerItemText}>{item.flat_number}</Text>
                  </TouchableOpacity>
                )}
                ListEmptyComponent={
                  <Text style={styles.modalHint}>{flatsLoading ? 'Loading…' : 'No flats found'}</Text>
                }
              />
            </View>
          </View>
