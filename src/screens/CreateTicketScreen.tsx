...
export default function CreateTicketScreen({
  ...
  route?: { params?: { kind?: TicketKind } };
}) {
  const { colors } = useTheme();
  const { user } = useAuth();
  const styles = useMemo(() => makeStyles(colors), [colors]);

  const kind: TicketKind = route?.params?.kind === 'bug' ? 'bug' : 'general';
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const finalTitle = isBug && !/^\[bug\]/i.test(t) ? `[BUG] ${t}` : t;
      ...
      Alert.alert('Success', isBug ? 'Bug report submitted.' : 'Ticket created.');
      navigation.goBack();
    } catch (e) {
      const msg =
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{isBug ? 'Report Software Bug' : 'Create Ticket'}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <TextInput
          style={styles.input}
          placeholder={isBug ? 'e.g. App crashes when opening Visitors' : 'Short title'}
        ...
        <View style={styles.buttonRow}>
          ...
          <TouchableOpacity style={[styles.button, loading && styles.buttonDisabled]} onPress={submit} disabled={loading}>
            {loading ? (
              ...
              <Text style={styles.buttonText}>{isBug ? 'Send bug report' : 'Submit'}</Text>
            )}
          </TouchableOpacity>
        </View>
      </ScrollView>
