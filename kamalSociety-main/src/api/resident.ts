import api, { logApiResponseSuccess, logFetchApiRequest } from './client';
import { setAuthToken, removeAuthToken } from '../lib/authTokenStorage';
...
export async function residentUpdate(id: string | number, data: ResidentUpdate, options?: ResidentUpdateOptions) {
  ...
  if (useMultipart) {
    ...
    if (data.society_id != null) fd.append('society_id', String(data.society_id));
    if (data.flat_id != null) fd.append('flat_id', String(data.flat_id));
...
/** Permanently delete the resident account (`DELETE /resident/delete/:id`). */
export async function residentDelete(id: string | number) {
  const res = await api.delete<{ success: boolean; message?: string }>(
    `/resident/delete/${encodeURIComponent(String(id))}`
  );
  return res.data;
}
